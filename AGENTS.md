...
- **Generated code is not committed to version control** — Freezed (`*.freezed.dart`), json_serializable (`*.g.dart`), Serverpod server (`apps/server/lib/src/generated/`), and Serverpod client (`packages/app_client/lib/src/protocol/`) outputs are gitignored and regenerated via `melos run generate`. The single established exception is the tracked mirror `apps/server/test/integration/test_tools/serverpod_test_tools.dart`, which is committed because the integration-test harness must compile without a generation step and is hash-gated by the same manifest as all generated output (see below).
