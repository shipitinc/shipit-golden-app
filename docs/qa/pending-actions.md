...
| 4 | Dark mode is broken in shipit_ui (`shipitDarkTheme()` uses static light surfaces with white text). Documented as UPSTREAM_UI_GAP-011 and reported as shipitinc/shipit-ui#10 (2026-09-08). When upstream ships dark tokens, bump the shipit_ui pin and add a dark-mode regression check. No app-side override (design authority + avoid drift). | shipit-ui upstream | reported |