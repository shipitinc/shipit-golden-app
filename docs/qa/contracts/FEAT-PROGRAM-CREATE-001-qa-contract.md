...
### Formal E_* Determination — E2E row (adopted AEF `01e0e84`, resolves shipit-golden-app#1)

**`evidence_determinations`:**

- `evidence_id: E2E_JOURNEY` — `determination: EXECUTED`; `artifact_ref:
  docs/qa/results/qa-session-e2e-live.md` + `evidence-e2e/`; `params:`
  `--dart-define=E2E_VERIFICATION_CODE=000000` (dev) +
  `SERVERPOD_DEV_VERIFICATION_CODE=000000` (dev-only server pin);
  `prerequisites: real Serverpod+PostgreSQL on :8097/:8096, browser
  live-interaction lane`; `reasons:` (n/a — EXECUTED); `evidence_refs:`
  e2e-01..e2e-05, DB records, server log.
- `evidence_id: E2E_AUTOMATED_RUNNER` (the `flutter test integration_test`
  device lane) — `determination: SKIPPED`; `reasons:` no headless device / CI
  device lane exists in this environment (iOS sim hangs at loading; web
  unsupported for `flutter test integration_test`); the journey is covered by
  the EXECUTED live-interaction row above and widget-level equivalents;
  `authority_ref:` upstream AEF#2 resolution (`01e0e84`) — REQUIRED-device
  gates formally determined rather than left `READY_NOT_EXECUTED` forever.

**`gate_results.e2e: PASS`** (live-interaction execution, artifact-tracked).
