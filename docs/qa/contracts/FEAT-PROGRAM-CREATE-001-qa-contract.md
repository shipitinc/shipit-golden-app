...
| E2E Journey (App→Server) | **READY_NOT_EXECUTED** | Contract-required journey addition (`§End-to-End Tests`) landed 2026-09-13: `apps/app/integration_test/app_journey_test.dart` gained the authenticated **create-program real-server journey** — register fresh account via UI → `/programs` → app-bar create entry point (`programs_create_action`) → fill form (name/description + calendar date entry) → submit → created program card in list → `View Details` → `JOINED` + Cancel Membership. Deterministic via `--dart-define=E2E_VERIFICATION_CODE` (dev server logs codes to stdout, pending-actions #2). Execution: **NOT RUN** in this environment — device-gated (no headless device; contract marks E2E NOT_IN_DEFAULT_PIPELINE). Not silently asserted; classified READY_NOT_EXECUTED per AEF. File compiles clean (`flutter analyze` no issues) and does not disturb `test:flutter` (121/121). |
