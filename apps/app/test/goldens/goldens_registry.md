...
| `goldens/program_create.png` | ProgramCreateScreen (initial empty form, idle) | DESIGN_PENDING | shipit_ui@c310a961aa + 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (FEAT-PROGRAM-CREATE-001, PC-009): initial empty create form, no error, idle — `shipitLightTheme()`, real Inter, `pumpAndSettle`, same conventions as `program_details_golden_test.dart`. Registered DESIGN_PENDING per frozen QA Contract AC-QA-009 / design revision §D-8 row 1 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS.|
| `goldens/programs_empty.png` | ProgramsScreen (empty state) | DESIGN_PENDING | shipit_ui@c310a961aa + 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (FEAT-PROGRAM-CREATE-001, PC-009): ProgramsScreen empty state — message `Programs you create will appear here.` + `Create program` CTA + app-bar actions incl. the create action. Registered DESIGN_PENDING per frozen QA Contract AC-QA-009 / design revision §D-8 row 2 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS.|
| `goldens/program_details_joined.png` | ProgramDetailsScreen (loaded, joined) | DESIGN_PENDING | shipit_ui@c310a961aa + 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (AC-11 / P2 gap #1 closure, FEAT-PROGRAM-CREATE-001): ProgramDetailsScreen loaded + **joined** state — `_MembershipCard` shows `Joined · <name>`, `JOINED` badge, `Cancel Membership` CTA; sits beside the not-joined `program_details.png`. Registered DESIGN_PENDING per frozen QA Contract AC-QA-011 / design revision §D-8 row 3 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS.|
