...
|------|--------|--------|-----------------|-------|
| `goldens/login_sign_in.png` | LoginScreen (sign in) | APPROVED | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Auth screen, shipit_ui components (approved design source per `product.yaml` `design.source: penpot`, implemented by the pinned shipit_ui revision). First promoted to APPROVED on 2026-09-08 against `shipit_ui@1207004`; re-approved on 2026-09-09 against `shipit_ui@d6abf9a` (the token-tree refactor migration re-renders these screens — regenerated on the Linux CI host via `.github/workflows/goldens-update.yml`, reviewer tariq). Baseline is rendered on the Linux CI host (authoritative golden runner) with the real bundled Inter TTFs; local macOS runs intentionally show a known ~1% rasterizer diff (CoreText vs FreeType) — never regenerate on macOS. REGENERATION REQUIRES design/human approval + registry update in the same commit. [re-approved 2026-09-10 for shipit_ui@18d1a5d6 by tariq via goldens-update.yml] [re-approved 2026-09-11 under the current pin shipit_ui@526926d — no pixel change; the pin bump consumed GAP-012/013/014 tokens at identical values, so the visual contract is re-affirmed without regeneration (see Pin note below)] [re-approved 2026-09-12 for shipit_ui@c310a961aa by tariq via goldens-update.yml] [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
| `goldens/login_register.png` | LoginScreen (register) | APPROVED | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Register/request-code mode. Auth screen, shipit_ui components (approved design source per `product.yaml` `design.source: penpot`, implemented by the pinned shipit_ui revision). First promoted to APPROVED on 2026-09-08 against `shipit_ui@1207004`; re-approved on 2026-09-09 against `shipit_ui@d6abf9a` (the token-tree refactor migration re-renders these screens — regenerated on the Linux CI host via `.github/workflows/goldens-update.yml`, reviewer tariq). Baseline is rendered on the Linux CI host (authoritative golden runner) with the real bundled Inter TTFs; local macOS runs intentionally show a known ~1% rasterizer diff (CoreText vs FreeType) — never regenerate on macOS. REGENERATION REQUIRES design/human approval + registry update in the same commit. [re-approved 2026-09-10 for shipit_ui@18d1a5d6 by tariq via goldens-update.yml] [re-approved 2026-09-11 under the current pin shipit_ui@526926d — no pixel change; the pin bump consumed GAP-012/013/014 tokens at identical values, so the visual contract is re-affirmed without regeneration (see Pin note below)] [re-approved 2026-09-12 for shipit_ui@c310a961aa by tariq via goldens-update.yml] [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
| `goldens/program_details.png` | ProgramDetailsScreen (loaded, not joined) | APPROVED | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Owner-approved detail surface (2026-09-12): membership status card + join CTA, shipit_ui components/tokens under the pinned `shipit_ui@c310a961aa`. Candidate bootstrapped on a dev host for registry conformance; the Linux-authoritative baseline is regenerated via `.github/workflows/goldens-update.yml` (dispatched 2026-09-12, reviewer tariq) which appends the approval marker to this row. See `docs/qa/pending-actions.md` #11. [re-approved 2026-09-12 for shipit_ui@c310a961aa by tariq via goldens-update.yml] [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
| `goldens/program_create.png` | ProgramCreateScreen (initial empty form, idle) | DESIGN_PENDING | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (FEAT-PROGRAM-CREATE-001, PC-009): initial empty create form, no error, idle — `shipitLightTheme()`, real Inter, `pumpAndSettle`, same conventions as `program_details_golden_test.dart`. Registered DESIGN_PENDING per frozen QA Contract AC-QA-009 / design revision §D-8 row 1 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS. [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
| `goldens/programs_empty.png` | ProgramsScreen (empty state) | DESIGN_PENDING | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (FEAT-PROGRAM-CREATE-001, PC-009): ProgramsScreen empty state — message `Programs you create will appear here.` + `Create program` CTA + app-bar actions incl. the create action. Registered DESIGN_PENDING per frozen QA Contract AC-QA-009 / design revision §D-8 row 2 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS. [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
| `goldens/program_details_joined.png` | ProgramDetailsScreen (loaded, joined) | DESIGN_PENDING | 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 | Candidate baseline preserving current rendering at HEAD `887efe3` (AC-11 / P2 gap #1 closure, FEAT-PROGRAM-CREATE-001): ProgramDetailsScreen loaded + **joined** state — `_MembershipCard` shows `Joined · <name>`, `JOINED` badge, `Cancel Membership` CTA; sits beside the not-joined `program_details.png`. Registered DESIGN_PENDING per frozen QA Contract AC-QA-011 / design revision §D-8 row 3 (`77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3`). Promotion to APPROVED is Linux-CI-only via `.github/workflows/goldens-update.yml` after design/human approval — NEVER regenerate on macOS. [re-approved 2026-09-13 for 77ccf5c1-9f2b-4798-87ec-c9b8bd7fb2e3 by tariq via goldens-update.yml]|
